...
LRESULT CALLBACK cbNewMain(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	switch (msg) {
				...
				if (cmd != IDM_SAVE_AS) {
					...
					if (saveFile(path16, filter16, TEXT(""), hWnd)) {
						...
						if (fp) {
							...
							MessageBox(hWnd, TEXT("Can't open file"), NULL, MB_OK);
						}
					}
				}
				free(buf);
		...
		case WM_NOTIFY : {
			...
			if (pHdr->idFrom == IDC_GRID && pHdr->code == (DWORD)LVN_KEYDOWN) {
				...
				if (!isCtrl &&
					...
					SendMessage(hWnd, WMU_EDIT_CELL, 0, kd->wVKey);
					return TRUE;
				}
		...
		// wParam = Is keep a current text?
		// lParam = The key that caused the message
		case WMU_EDIT_CELL: {
			...
			SetProp(hEdit, TEXT("AUTOCOMPLETE"), (HANDLE)GetProp(hWnd, TEXT("AUTOCOMPLETE")));
			...
			*pHasRowid = sqlite3_exec(db, query8, 0, 0, 0) == SQLITE_OK;
			...
			if (getStoredValue(TEXT("editable"), 1)) {
				...
				if (*pHasRowid) {
					...
					InsertMenu(hGridMenu, IDM_SEPARATOR2, MF_BYCOMMAND | MF_STRING, IDM_ADD_ROW, TEXT("Add row\tIns"));
					InsertMenu(hGridMenu, IDM_SEPARATOR2, MF_BYCOMMAND | MF_STRING, IDM_DELETE_ROW, TEXT("Delete row(s)\tDel"));
				}
			}
		...
		case WMU_HOT_KEYS: {
			...
			if (wParam == 0x48 && isCtrl) { // Ctrl + H
				SendMessage(hWnd, WM_COMMAND, IDM_HIDE_COLUMN, 0);
				return TRUE;
			}

			if (wParam == 0x46 && isCtrl) { // Ctrl + F
				SendMessage(hWnd, WM_COMMAND, IDM_FILTER_ROW, 0);
				return TRUE;
			}

			if (wParam == 0x53 && isCtrl) {// Ctrl + S
			...
			if (isCtrl && wParam >= 0x30 && wParam <= 0x39) {// 0-9
				HWND hGridWnd = GetDlgItem(hWnd, IDC_GRID);
				HWND hHeader = ListView_GetHeader(hGridWnd);
				int colCount = Header_GetItemCount(ListView_GetHeader(hGridWnd));
...
LRESULT CALLBACK cbNewDataGrid(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	...
	if (msg == WM_COMMAND) {
		if (HIWORD(wParam) == EN_CHANGE && LOWORD(wParam) == IDC_CELL_EDIT) {
	...
	switch(msg){
		...
		case WM_KEYDOWN: {
			...
			if (wParam == VK_UP || wParam == VK_DOWN) {
				SetFocus(GetDlgItem(hMainWnd, IDC_GRID));
	...
	switch (msg) {
		...
		case WMU_AUTOCOMPLETE: {
			...
			if (hWnd != GetProp(hAutoComplete, TEXT("CURRENTINPUT"))) {
				...
				SendMessage(hAutoComplete, WM_SETFONT, (WPARAM)hFont, TRUE);
			...
			HWND hMainWnd = GetDlgItem(GetParent(hAutoComplete), IDC_MAIN);
			...
			int colNo = (int)(LONG_PTR)GetProp(hWnd, TEXT("COLNO"));
			...
			char fkTableName8[MAX_TABLE_LENGTH + 1] = {0};
			...
			if (SQLITE_OK == sqlite3_prepare_v2(db, "select \"table\", \"to\" from pragma_foreign_key_list(?1) where \"from\" = ?2", -1, &stmt, 0)) {
				...
				sqlite3_bind_text(stmt, 2, colName8, strlen(colName8), SQLITE_TRANSIENT);

				if (SQLITE_ROW == sqlite3_step(stmt)) {
					strncpy(fkTableName8, (const char*)sqlite3_column_text(stmt, 0), MAX_TABLE_LENGTH);
					strncpy(fkColName8, (const char*)sqlite3_column_text(stmt, 1), MAX_TABLE_LENGTH);
				}
			}
			sqlite3_finalize(stmt);

			int maxRows = getStoredValue(TEXT("max-autocomplete-results"), 1000);
...
LRESULT CALLBACK cbNewCellEdit(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	...
	switch (msg) {
		...
		case WM_KILLFOCUS: {
			if ((HWND)wParam != (HWND)GetProp(hWnd, TEXT("AUTOCOMPLETE")))
				DestroyWindow(hWnd);
		}
		break;
...
LRESULT CALLBACK cbNewAutoComplete(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	switch (msg) {
		case WM_LBUTTONUP: {
			...
			SetProp(hEdit, TEXT("CHANGEBYAUTOCOMPLETE"), (HANDLE)1);
			SetWindowText(hEdit, buf);
			int len = _tcslen(buf);
			SendMessage(hEdit, EM_SETSEL, len, len);
			SetFocus(hEdit);
			UpdateWindow(GetParent(hEdit));
		}
		break;
	}
